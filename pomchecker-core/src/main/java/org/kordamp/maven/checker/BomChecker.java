...
public class BomChecker {
    ...
    public static void check(Logger log, MavenProject project, Configuration configuration) throws PomCheckException {
        ...
        Model model = project.getRawModel().orElseThrow(() -> new IllegalStateException("Raw model not present"));
        ...
        if (!errors.isEmpty()) {
            ...
            log.info("BOM {} passes all checks.", project.getPom().toFile().getAbsolutePath());
        }
    }
}
